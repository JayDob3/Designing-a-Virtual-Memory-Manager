...
public class VM
{
	...
	/* List of all the private members of the class that will be used by codes from within same class  */
	...
	/* Method which returns nextFrameNumber value */
   
	public int getNextFrame() {
		return nextFrameNumber++;
	}

	/* Method which will check/track pageNumber transactions  */
   
	public int checkTLB(int pageNumber) {
		int frameNumber = -1;

		/* Method to insert a pageNumber and FrameNumber into TLB */
	...
	public void setTLBMapping(int pageNumber, int frameNumber) {
		...
		/* Method which updates the next TLB entry */
	...
	public int getPhysicalAddress(int virtualAddress) throws java.io.IOException {
		...
		/* Method which tries to resolves TLB miss and corrects frame number */
		...
		if ( (frameNumber = checkTLB(pageNumber)) == -1 ) {  /** TLB Miss **/
			...
			if (pageTable[pageNumber].getValidBit() == true) {
				...
				/* Method procedure which handles page fault  */
	...
	public byte getValue(int physicalAddress) throws java.io.IOException {
		...
		/* Method which returns physicalMemory value */
